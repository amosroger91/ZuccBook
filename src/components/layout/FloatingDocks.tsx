  ...
  return (
    <Box sx={{ ...PANEL_SX, display: visible ? "flex" : "none" }}>
      ...
      <Box sx={{ flex: 1, minHeight: 0, position: "relative", display: "flex", flexDirection: "column" }}>
        <Box ref={scrollRef} sx={{ flex: 1, overflowY: "auto", p: 1.25, display: "flex", flexDirection: "column", gap: 0.75, minHeight: 0 }}>
        ...
        {pending > 0 && (
          <Button onClick={() => jump()} size="small" startIcon={<KeyboardArrowDownRoundedIcon />}
            sx={{ position: "absolute", bottom: 8, left: "50%", transform: "translateX(-50%)", borderRadius: 999, textTransform: "none", fontWeight: 800, fontSize: 12, px: 1.5, py: 0.3, minHeight: 0, boxShadow: 4, color: "#fff", background: "linear-gradient(135deg,#2bb673,#159e63)", "&:hover": { background: "linear-gradient(135deg,#2bb673,#0e8754)" } }}>
            {pending} new
          </Button>
        )}
      </Box>
