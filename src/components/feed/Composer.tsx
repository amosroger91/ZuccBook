        ...
        <Box sx={{ flex: 1 }}>
          ...
          {media.length > 0 && (
            <Stack direction="row" spacing={1} sx={{ mt: 1, flexWrap: "wrap" }}>
              {media.map((m, i) => (
                <Box key={i} component="img" src={m.url} sx={{ width: 84, height: 84, objectFit: "cover", borderRadius: 2, border: "1px solid rgba(58,155,240,0.2)" }} onClick={() => setMedia((x) => x.filter((_, j) => j !== i))} />
              ))}
            </Stack>
          )}
